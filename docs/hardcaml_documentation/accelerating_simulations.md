...
behavioral model in C++ or SystemC that runs with high
...
[Verilator](https://www.veripool.org/wiki/verilator) and g++ has to be
installed on your system.

The key benefit for using the Verilator-based backend is that it is
...
The primary benefit from using Hardcaml_c is that it has a much more
modest compilation time (compared to `Hardcaml_verilator`), but still
much more performant than the regular Hardcaml Cyclesim simulator.

To use `Hardcaml_c`, `gcc` has to be installed in your local machine.
...
Similar to Hardcaml Verilator, Hardcaml C supports a Interface-based
